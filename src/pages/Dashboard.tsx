...
const Dashboard = () => {
  ...
  const fetchAdvice = useCallback(async () => {
    ...
    // Aucun conseil pour aujourd'hui → générer automatiquement (une seule fois)
    ...
    try {
      ...
      if (fresh && fresh.length > 0) {
        const ORDER: Record<string, number> = { alerte: 0, warning: 0, astuce: 1, observation: 2 };
        setAdvices([...fresh].sort((a: any, b: any) => (ORDER[a.advice_group] ?? 3) - (ORDER[b.advice_group] ?? 3)));
      }
    } catch { /* silencieux */ }
    finally { setAdviceGenerating(false); }
  }, []);
          ...
          {advices.length > 0 && (
            <>
              <p className="text-[11px] text-muted-foreground text-center">
                {routineRefreshAvailable
                  ? "Nouveau conseil disponible"
                  : `Nouveau conseil dans ${routineRefreshHoursLeft}h`}
              </p>
              <button
