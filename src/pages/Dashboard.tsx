      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}
        ...
        {(() => {
          ...
          const handleDragEnd = (e: any, info: any) => {
            if (info.offset.x < -100) setActiveCard(prev => (prev + 1) % count);
            else if (info.offset.x > 100) setActiveCard(prev => (prev - 1 + count) % count);
          };
            ...
            <div className="relative py-12 flex justify-center items-center" style={{ height: CARD_H + 60 }}>
              <AnimatePresence initial={false}>
                {adviceList.map((advice, i) => {
                  ...
                  if (rel > 2 && rel < count - 1) return null; // Only show top 3 cards for performance and stack look
                  ...
                  return (
                    <motion.div
                      ...
                      exit={{ opacity: 0, scale: 0.8, x: xOffset > 0 ? 200 : -200 }}
                      ...
                      onClick={() => isTop && setSelectedTip(advice)}
                      ...
                      <div className="flex items-center justify-between border-t border-primary/5 pt-6">
                        ...
                        <div className="flex gap-1.5">
                          {adviceList.map((_, dotIdx) => (
                            <div key={dotIdx} className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${dotIdx === i ? 'bg-primary w-4' : 'bg-primary/10'}`} />
                          ))}
                        </div>
                      </div>
                    </motion.div>
