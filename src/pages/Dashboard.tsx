...
const Dashboard = () => {
  ...
  // ── Dernière génération routine/conseils (cooldown 24h) ─────────────────────
  const fetchLastRoutineRefresh = useCallback(async () => {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session?.user) return;
    const today = new Date().toISOString().split("T")[0];
  ...
  useEffect(() => { fetchLastRoutineRefresh(); }, [fetchLastRoutineRefresh]);
  ...
  const handleRegenerate = async () => {
    if (!routineRefreshAvailable || regenerating) return;
        ...
        {!routineTreated && routineProducts.length > 0 ? (
          ...
          <div className="mb-3">
            ...
            <button
              ...
              disabled={regenerating || !routineRefreshAvailable}
