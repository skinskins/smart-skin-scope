...
export const FACTOR_TAG_KEYS = [
    "poor_sleep", "good_sleep", "high_stress", "serene", "high_sugar", "low_water",
    "balanced_diet", "sport", "sedentary", "sun", "screens", "smoking", "hormonal",
] as const;
...
const StepFactors = ({ BackButton, defaultFactors = [], addDefaultFactor, removeDefaultFactor }: SignupStepProps) => {
    ...
    const absorbTag = (key: string) => {
        addDefaultFactor?.(key);
        pulseOrb();
    };

    const visibleTags = TAGS.filter(t => !defaultFactors.includes(t.key));
        ...
        <>
            <div className="mb-6 flex items-start gap-4">
                ...
                <div>
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Glisse vers Nacre ce qui façonne ta peau</p>
                </div>
            </div>
                ...
                {defaultFactors.length > 0 && (
                    ...
                    >
                        ...
                        <div className="flex flex-wrap gap-2 justify-center">
                            {defaultFactors.map(key => {
                                const t = TAGS.find(t => t.key === key)!;
                                return (
                                    <button
                                        key={key}
                                        type="button"
                                        onClick={() => removeDefaultFactor?.(key)}
